...
def calculate_lateral_flow(L, h, Cw=1.84, N=100, nt=len(Q_at_x0)):
    """
    Args:
...
def sinuo_momen(real_length, study_length):
    return real_length/study_length
...
def calculateDischarge(g, A, W, dx, dt, n, L, beta, Q_upstream, qL, u, sm=[1,1], N=100):
    ...
    Sm = sinuo_momen(sm[0], sm[1])  # Sinuosity coefficient for momentum equation
