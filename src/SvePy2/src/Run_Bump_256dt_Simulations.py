#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 23 11:48:49 2018

Simulations for flow over a bump
Using 256 grid cells (128 in domain)
Modifying CFL limits to look at convergence with decreasing dt

@author: brh
"""

def main(argv):
    
    import SimulationRun as sr
    import numpy as np
    
    #--------------------------------------------------------------------------                       
    # RUN-TIME OUTPUT CONTROLS
    # useful for compact printout during debugging
    np.set_printoptions(precision=10)


   #--------------------------------------------------------------------------
    # SETUP  
    # Provide a dictionary that replaces the methods of setting.###
    #   with the values in the dictionary. See SettingDefault.py for values
    # At a minimum, the 'user_case_name' must be provided that
    #   matches one of the predefined cases. 
    #   See SimulationRun.predefined_case_setup()               
    
    # Waller Creek baseline - starting from time 0
#    custom = {'user_case_name'     : 'Waller_Creek_baseline', 
#              'plot_element_limits': [190,250],
#              'plot_types'         : ['free_surface','flowrate']}
    
    #--------------------------------------------------------------------------
    # new setup

    custom = {'user_case_name'     : 'flow_over_a_bump_0256a', 
              'txtout_directory'   : 'outtxt_bump_0256a',
              'binsave_directory'  : 'outbin_bump_0256a',
              'flow_over_a_bump_NX': 256,
              'heightBC': 0.262,
              'time_total': 400,
              'plot_time_interval' : 4,
              'print_time_header_step_interval':100,
              'cfl_max':  0.7,
              'cfl_increase_dt': 0.6,
              'mannings_n_global_default': 0.0,
              'mannings_n_for_buffer': 0.03,
              'method_momentum' :  'nonuniform',
              'method_hydjump_face': 'simple',
              'method_Q_damp_oscillation_coef': 0.1,
              'method_Q_damp_oscillation_type': 'linear_simple', 
              'method_Q_adjust_Vshape' : True,
              'method_Q_adjust_Vshape_coef' :1.0,
              'plot_element_limits': [50,75],            
              'plot_types'         : ['free_surface','flowrate'],
              'IC_type'              :'restart_file',        
              'restart_time'         : 290,
              'restart_time_units'   :'seconds',
              'restart_volumefile'   :   'volume_output_N0256_20180423_1121.txt',
              'restart_flowratefile' :   'flowrate_output_N0256_20180423_1121.txt',
              'restart_directory'    : 'restart',
              'swashes_bin'        : '/home/marie/anaconda3/envs/Hydro/bin/swashes'}

    #--------------------------------------------------------------------------
    # run the simulation
    [trun, setting] = sr.SimulationRun().simulation_toplevel(custom)
    
    del trun
    del setting

    
    #--------------------------------------------------------------------------
    # new setup
    custom = {'user_case_name'     : 'flow_over_a_bump_0256b', 
              'txtout_directory'   : 'outtxt_bump_0256b',
              'binsave_directory'  : 'outbin_bump_0256b',
              'flow_over_a_bump_NX': 256,
              'heightBC': 0.262,
              'time_total': 400,
              'plot_time_interval' : 4,
              'print_time_header_step_interval':100,
              'cfl_max':  0.35,
              'cfl_increase_dt': 0.3,
              'mannings_n_global_default': 0.0,
              'mannings_n_for_buffer': 0.03,
              'method_momentum' :  'nonuniform',
              'method_hydjump_face': 'simple',
              'method_Q_damp_oscillation_coef': 0.1,
              'method_Q_damp_oscillation_type': 'linear_simple', 
              'method_Q_adjust_Vshape' : True,
              'method_Q_adjust_Vshape_coef' :1.0,
              'plot_element_limits': [50,75],            
              'plot_types'         : ['free_surface','flowrate'],
              'IC_type'              :'restart_file',        
              'restart_time'         : 290,
              'restart_time_units'   :'seconds',
              'restart_volumefile'   :   'volume_output_N0256_20180423_1121.txt',
              'restart_flowratefile' :   'flowrate_output_N0256_20180423_1121.txt',
              'restart_directory'    : 'restart',
              'swashes_bin'        : '/home/marie/anaconda3/envs/Hydro/bin/swashes'}

    #--------------------------------------------------------------------------
    # run the simulation
    [trun, setting] = sr.SimulationRun().simulation_toplevel(custom)
    
    del trun
    del setting

    #--------------------------------------------------------------------------
    # new setup

    custom = {'user_case_name'     : 'flow_over_a_bump_0256c', 
              'txtout_directory'   : 'outtxt_bump_0256c',
              'binsave_directory'  : 'outbin_bump_0256c',
              'flow_over_a_bump_NX': 256,
              'heightBC': 0.262,
              'time_total': 400,
              'plot_time_interval' : 4,
              'print_time_header_step_interval':100,
              'cfl_max':  0.1,
              'cfl_increase_dt': 0.085,
              'mannings_n_global_default': 0.0,
              'mannings_n_for_buffer': 0.03,
              'method_momentum' :  'nonuniform',
              'method_hydjump_face': 'simple',
              'method_Q_damp_oscillation_coef': 0.1,
              'method_Q_damp_oscillation_type': 'linear_simple', 
              'method_Q_adjust_Vshape' : True,
              'method_Q_adjust_Vshape_coef' :1.0,
              'plot_element_limits': [50,75],            
              'plot_types'         : ['free_surface','flowrate'],
              'IC_type'              :'restart_file',        
              'restart_time'         : 290,
              'restart_time_units'   :'seconds',
              'restart_volumefile'   :   'volume_output_N0256_20180423_1121.txt',
              'restart_flowratefile' :   'flowrate_output_N0256_20180423_1121.txt',
              'restart_directory'    : 'restart',
              'swashes_bin'        : '/home/marie/anaconda3/envs/Hydro/bin/swashes'}

    #--------------------------------------------------------------------------
    # run the simulation
    [trun, setting] = sr.SimulationRun().simulation_toplevel(custom)
    
    del trun
    del setting

    #--------------------------------------------------------------------------
    # new setup

    custom = {'user_case_name'     : 'flow_over_a_bump_0256d', 
              'txtout_directory'   : 'outtxt_bump_0256d',
              'binsave_directory'  : 'outbin_bump_0256d',
              'flow_over_a_bump_NX': 256,
              'heightBC': 0.262,
              'time_total': 400,
              'plot_time_interval' : 4,
              'print_time_header_step_interval':100,
              'cfl_max':  0.035,
              'cfl_increase_dt': 0.03,
              'mannings_n_global_default': 0.0,
              'mannings_n_for_buffer': 0.03,
              'method_momentum' :  'nonuniform',
              'method_hydjump_face': 'simple',
              'method_Q_damp_oscillation_coef': 0.1,
              'method_Q_damp_oscillation_type': 'linear_simple', 
              'method_Q_adjust_Vshape' : True,
              'method_Q_adjust_Vshape_coef' :1.0,
              'plot_element_limits': [50,75],            
              'plot_types'         : ['free_surface','flowrate'],
              'IC_type'              :'restart_file',        
              'restart_time'         : 290,
              'restart_time_units'   :'seconds',
              'restart_volumefile'   :   'volume_output_N0256_20180423_1121.txt',
              'restart_flowratefile' :   'flowrate_output_N0256_20180423_1121.txt',
              'restart_directory'    : 'restart',
              'swashes_bin'        : '/home/marie/anaconda3/envs/Hydro/bin/swashes'}

    #--------------------------------------------------------------------------
    # run the simulation
    [trun, setting] = sr.SimulationRun().simulation_toplevel(custom)
    
    del trun
    del setting


if __name__ == '__main__':
    import sys    
    main(sys.argv) 
    
#==============================================================================
#EOF
